...
- Routes are split by domain: `auth`, `geo`, `tracking`, `messages`, `pitstops`, `safeZones`, `itinerary`, `demo`, `trips`, `health` — each a self-contained Express router.
...
- Single schema file [lib/db/src/schema/convoy.ts](lib/db/src/schema/convoy.ts): `users`, `trips`, `trip_members`, `messages`, `pitstops`, `pitstop_responses`, `safe_zones`, `itinerary_items`, `location_history`. Drizzle-zod generates `insert*Schema` + row types alongside each table.
...
- `itinerary_items` holds the group itinerary (`kind`: `stay | activity | transport | food | other`, `startAt`/optional `endAt`, optional address/confirmation code/notes), soft-deleted via `status` (`active | removed`). Any joined member can add; creator or leader can edit/remove (`itinerary.ts`); read-only once the trip ends.
...
- Expo Router app; `app/` holds file-based routes (`trips.tsx`, `create-trip.tsx`, `join.tsx`, `signin.tsx`, `trip/[id]/{index,tracking,messages,itinerary}.tsx`).
